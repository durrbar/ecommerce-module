...
use Modules\Ecommerce\Models\Availability;
...
class ProductRepository extends BaseRepository
{
        ...
        try {
            ...
            if (isset($request['variation_options'])) {
                ...
                if (isset($request['variation_options']['delete'])) {
                    foreach ($request['variation_options']['delete'] as $id) {
                        try {
    ...
    public function fetchRelated($slug, $limit = 10, $language = DEFAULT_LANGUAGE)
    {
        try {
            ...
            return $this->where('language', $language)->whereHas('categories', function ($query) use ($categories): void {
    ...
    {
        return Availability::where('bookable_id', $variation_id)->where('bookable_type', 'Modules\Ecommerce\Models\Variation')->whereDate('from', '>=', $from)->whereDate('to', '<=', $to)->get();
    }
